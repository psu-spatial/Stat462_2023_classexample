...
In our case, the intercept is the *average* mass of penguins with no
flippers (zero flipper length), which corresponds to
`r Model1.Summary$betas[1]` grams.

This is extrapolating far beyond the observed data and thus has no
physical meaning. Equally, a penguin with no flippers is likely to have
other physical differences which impact how much it weighs. The
relationship is no longer likely to be linear.
...
The slope suggests that as the flipper length increases by 1mm, the mass
increases by `r Model1.Summary$betas[2]` grams.
...
We can see from this model summary that `r Model1.Summary$rsq*100`% of
the variability in our sampled Penguin mass can be explained by flipper
length.
...
**Is there a linear relationship between mass and flipper length? (AKA
is** $\beta_{1} = 0$?)

See here for a worked example in detail:
...
If x does not impact y, then the slope of our *population scatterplot*
would be zero. E.g. x does not explain any of the variability in y. So
in this case, we are assessing how likely it is to see our sample in a
world where the TRUE population slope is zero.

So for our null hypothesis, we set up a world where the population slope
is zero. We don't care if there is a negative or a positive slope here,
so this is a *two sided* T-test.

-   H_0\_ : $\beta_{1} = 0$
-   H_1\_ : $\beta_{1} != 0$ *AKA our sample has an unusual slope if H0
    really was true*
...
This is such a common question that you can simply see this T value in
the R summary output. THE P-VALUE IN THIS IS FOR A TWO SIDED TEST.

```{r}
summary(Model1)
```

We can see that the probability is exceedingly small, so we have a lot
of evidence to reject H_0\_ and suggest there is a relationship between
...
**Is there a relationship between body mass and study year, at a 10%
...
You can see from this that the summary t-statistic is -0.358 and the
associated p value is 0.721. Let's show where it comes from.
...
Here we can see that 36% of the T-distribution is on the lower side of
the test statistic (e.g. by random chance we expect 36% of slopes to be
smaller than -16.37) and 64% of the T-distribution is on the upper side
of the test statistic. As the red line is in the white zone, we do not
...
Alternatively, as the T-distribution is symmetrical, we can simply
...
E.g. 0.721 is greater than 0.1 so we do not have enough evidence to
reject H0. This is what the R summary does.

SO R-SUMMARY T-TEST STATISTICS ARE TWO SIDED e.g. H1: B1 != 0 or B0 !=0
You can actually see this directly in the summary, Pr(\>\|t\|) e.g. the
p value for negative t and positive t.
...
The p-value is 0.08 - as this is above 0.05, we do not have enough
evidence to reject H0. E.g.
...
It's a bit more convoluted, but just as easy to use a confidence
interval instead of a t-test. Is the slope 0 at a critical significance
of 5%?
...
The ols regress output gives you the 95% confidence interval on the
slope. AKA What range of values you expect the TRUE population slope to
lie between.

We can see that the 95% confidence interval ON THE SLOPE is between
22.801 and 42.862 mm/g. As there are values greater than 35 mm/g inside
the confidence interval, it would be reasonable to agree with the
scientist's theory. E.g. we don't have evidence to suggest our sample
slope would always be lower than 40 mm/g.
...
**For our model 1, is there evidence to suggest that penguins with no
flippers have greater than zero weight, with a critical significance of
1%?**
...
If a penguin has no flippers then x = 0 and mass = $\beta_0$. So the
*Intercept* of our *population scatterplot* would be zero. So in this
case, we are assessing how likely it is to see our sample in a world
where the TRUE population intercept is zero.
...
For our null hypothesis, we set up a world where the population
intercept is zero. We don't care if there is a negative or a positive
intercept here, so this is a *two sided* T-test.

-   H_0\_ : $\beta_{0} = 0$
-   H_1\_ : $\beta_{1} != 0$ *AKA our sample has an unusual intercept if
    H0 really was true*

$$
t = \frac{-2535.837 − 0}{964.798}=-2.628
$$ And because the test is two sided, we look at 2 x the associated p
value (see slope version for why)

```{r}
2*pt(-2.628,df=149)
```

As our p value of 0.009 is *just* below 0.01, we have enough evidence to
reject the null hypothesis and suggest that the intercept is not zero.
JUST!
...
So you can see that the range of values for the intercept does not
include zero at 99%.

We can again get the t-test statistic and p value from the R output
...
These are VERY similar to the slope ones, so you can use those as
examples, More on the class textbook website also.
